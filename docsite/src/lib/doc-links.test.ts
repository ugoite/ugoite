...
test("REQ-E2E-006: resolveDocHref resolves absolute docsite and application routes against the configured base path", async () => {
	...
	expect(resolveDocHref("/docs/spec/index", "guide/cli.md")).toBe(
		"/ugoite/docs/spec/index",
	);
	expect(resolveDocHref("/docs/spec/ui", "guide/cli.md")).toBe(
		"/ugoite/docs/spec/ui/README",
	);
	expect(resolveDocHref("/docs/spec/ui/README.md#nav", "guide/cli.md")).toBe(
		"/ugoite/docs/spec/ui/README#nav",
	);
	expect(resolveDocHref("/docs/spec/architecture", "guide/cli.md")).toBe(
		"https://github.com/octo/example/tree/main/docs/spec/architecture",
	);
	expect(resolveDocHref("/docs/missing", "guide/cli.md")).toBeNull();
	expect(
		resolveDocHref(
...
test("REQ-E2E-006: resolveDocHref resolves relative docs and repository source links without leaking outside the base path", async () => {
	...
	try {
		expect(
			resolveDocHref("../index.md?tab=api#intro", "spec/api/rest.md"),
		).toBe("/ugoite/docs/spec/index?tab=api#intro");
		expect(resolveDocHref("../architecture", "spec/api/rest.md")).toBe(
			"https://github.com/octo/example/tree/main/docs/spec/architecture",
		);
		expect(resolveDocHref("../missing-dir", "spec/api/rest.md")).toBeNull();
		expect(resolveDocHref("../../README.md?plain=1", "guide/cli.md")).toBe(
			"https://github.com/octo/example/blob/main/README.md?plain=1",
		);
		expect(resolveDocHref("../../", "guide/cli.md")).toBe(
			"https://github.com/octo/example/blob/main/README.md",
		);
		expect(resolveDocHref("../../missing.txt", "guide/cli.md")).toBeNull();
		expect(resolveDocHref("../../dev-null-link", "guide/cli.md")).toBeNull();
	} finally {
		rmSync(devNullLinkPath, { force: true });
	}
});
