...
mod tests {
    ...
    fn list_tables_are_borderless_and_role_aware() {
        ...
        let plain = render_list_table("LOCAL_SPACE_PATH", &items, &StylePolicy::new(false));
        assert_eq!(plain, "LOCAL_SPACE_PATH\nteam-notes\nresearch\nscratch");
        assert!(!plain
            .lines()
            .any(|line| line.chars().all(|character| character == '-')));

        let styled = render_list_table("LOCAL_SPACE_PATH", &items, &StylePolicy::new(true));
        assert_eq!(strip_ansi(&styled), plain);
        assert!(styled.contains("\u{1b}[2mLOCAL_SPACE_PATH\u{1b}[0m"));
