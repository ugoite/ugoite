...
async fn test_remote_asset_second_upload_returns_reference() {
    ...
    assert_eq!(first_asset["kind"].as_str(), Some("asset"));
    assert_eq!(second_asset["kind"].as_str(), Some("asset"));
...
async fn test_lane1_parity_fixture_converges_on_cli_remote() {
    ...
    let asset_id = receipt["id"]
        .as_str()
        .expect("upload returns asset id")
        .to_string();
    // Upload machine output is the stable receipt; the attach path needs the
    // stored reference object, rebuilt here from the known upload metadata.
    let mut hasher = Sha256::new();
    hasher.update(b"spec-bytes");
