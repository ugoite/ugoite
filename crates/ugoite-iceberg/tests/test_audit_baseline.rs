//! Temporary characterization of audit F01, not evidence that it is fixed.
//! Replace the inverse mismatch assertions with equality in the integrity fix.
...
async fn audit_baseline_revert_retains_after_integrity() -> Result<()> {
    ...
    assert_eq!(inverse["parent_revision_id"], after_id);
    assert_eq!(inverse["change_id"], receipt["change_id"]);
    let (checksum, signature) = independent_integrity(&current.markdown, &key);
    // Observed F01 defect: the new body is correct, but both integrity values
    // still describe the replaced revision. The fix must make these equal.
    assert_ne!(inverse["integrity"]["checksum"], checksum);
    assert_ne!(inverse["integrity"]["signature"], signature);
    assert_eq!(
        inverse["integrity"]["checksum"],
        after["integrity"]["checksum"]
    );
    assert_eq!(
        inverse["integrity"]["signature"],
        after["integrity"]["signature"]
    );
    assert_eq!(
        service
