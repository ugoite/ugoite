...
`ugoite-domain` is the portable Rust layer that stays small enough for
`wasm32-unknown-unknown` builds and other lightweight consumers.

## Responsibility boundary

`ugoite-domain` owns portable building blocks:
...
`ugoite-domain`. If it depends on storage adapters, indexing engines, or FFI,
...
```bash
mise run //ugoite-domain:build:wasm
```

If you are already inside `ugoite-domain/`, `mise run build:wasm` runs the
same task.

## Why only small extractions move here

`compute_word_count` now lives in `ugoite-domain` because it is a portable text
