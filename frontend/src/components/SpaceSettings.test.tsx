...
describe("SpaceSettings", () => {
  ...
  it("should display storage config", () => {
    render(() => <SpaceSettings space={mockSpace} onSave={vi.fn()} />);
    expect(screen.getByDisplayValue("s3://planned-bucket/test-space"))
      .toBeInTheDocument();
    expect(screen.getByDisplayValue("https://s3.example.com"))
      .toBeInTheDocument();
  });
  ...
  it("should save storage config", async () => {
    ...
    fireEvent.input(uriInput, {
      target: { value: "file:///var/lib/ugoite/migrated" },
    });
  ...
  it("should save a new local storage config when no existing config is present", async () => {
    ...
    const uriInput = screen.getByLabelText(/storage uri/i);
    fireEvent.input(uriInput, {
      target: { value: "file:///var/lib/ugoite/fresh" },
    });
  ...
  it("test connection button not shown when onTestConnection not provided", () => {
    render(() => <SpaceSettings space={mockSpace} onSave={vi.fn()} />);
    expect(screen.queryByRole("button", { name: /test connection/i })).not
      .toBeInTheDocument();
  });

  it("REQ-FE-017: explains that saved storage URIs are metadata-only before migration", () => {
    render(() => <SpaceSettings space={mockSpace} onSave={vi.fn()} />);
    expect(screen.getByText(/saved uri below is migration metadata/i))
      .toBeInTheDocument();
    expect(
      screen.getByText(
        /backend still writes through the deployment-wide storage root/i,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
  ...
  it("REQ-FE-017: frames connector URIs as future migration targets", () => {
    ...
    expect(
      screen.getByText(
        /records a local path you may want to migrate this space to later/i,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
    ...
    expect(
      screen.getByText(
        /team access and backups, but it adds cloud credentials and usage costs/i,
      ),
    ).toBeInTheDocument();
    expect(screen.getByRole("link", { name: /storage migration guide/i }))
      .toHaveAttribute(
        "href",
  ...
  it("REQ-FE-060: settings show the current storage topology before editing", () => {
    ...
    expect(screen.getByText("file:///var/lib/ugoite/current"))
      .toBeInTheDocument();
    expect(screen.getByDisplayValue("s3://planned-bucket/test-space"))
      .toBeInTheDocument();
  });
