...
export const handlers = [
  ...
  // Test connection
  testHttp.post(
    "/spaces/:spaceId/test-connection",
  ...
  testHttp.post("/spaces/:spaceId/entries", async ({ params, request }) => {
    ...
    return HttpResponse.json({ id: entryId, revision_id: revisionId }, {
      status: 201,
    });
  }),
  ...
  testHttp.put(
    ...
    async ({ params, request }) => {
      ...
      const entry = mockEntries.get(spaceId)?.get(entryId);
      if (!entry) {
        return HttpResponse.json({ detail: "Entry not found" }, {
          status: 404,
        });
      }
  ...
  testHttp.get(
    "/spaces/:spaceId/entries/:entryId/history/:revisionId",
    ({ params }) => {
      const spaceId = params.spaceId as string;
      const entryId = params.entryId as string;
      ...
      if (!entry || entry.revision_id !== revisionId) {
        return HttpResponse.json({ detail: "Revision not found" }, {
          status: 404,
        });
      }
      return HttpResponse.json(normalizeMockEntry(entry));
  ...
  // Restore entry
  testHttp.post(
    "/spaces/:spaceId/entries/:entryId/restore",
    async ({ params, request }) => {
      const spaceId = params.spaceId as string;
      const entryId = params.entryId as string;
      const entry = mockEntries.get(spaceId)?.get(entryId);
  ...
  testHttp.post(
    "/spaces/:spaceId/members/invitations",
    async ({ params, request }) => {
      const spaceId = params.spaceId as string;
      if (!mockSpaces.has(spaceId)) {
        return HttpResponse.json({ detail: "Not found" }, { status: 404 });
