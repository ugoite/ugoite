...
export const getConfiguredFrontendTestOrigin = (): string =>
  getFrontendTestOrigin();

export const getConfiguredFrontendTestApiBase = (): string =>
  getFrontendTestApiBase();

const normalizeTestPath = (
  path = "/",
): string => (path.startsWith("/") ? path : `/${path}`);

export const testApiPath = (path = "/"): string =>
  `/api${normalizeTestPath(path)}`;
