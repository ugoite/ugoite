...
export function isReservedAdminSpace(
  space: Pick<Space, "id" | "is_admin_space">,
): boolean {
...
export function partitionSpaces(spaces: readonly Space[]): {
  ...
  const userSpaces = sortedSpaces.filter((space) =>
    !isReservedAdminSpace(space)
  );
  const adminSpaces = sortedSpaces.filter((space) =>
    isReservedAdminSpace(space)
  );
