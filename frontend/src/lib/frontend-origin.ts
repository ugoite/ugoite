...
export const getFrontendTestOrigin = (env = getEnvMap()): string =>
  normalizeOrigin(
    readEnvValue(env, ["FRONTEND_TEST_ORIGIN"]) ?? defaultFrontendOrigin,
  );

export const getRuntimeFrontendOrigin = (env = getEnvMap()): string =>
  normalizeOrigin(
    readEnvValue(env, ["FRONTEND_ORIGIN", "ORIGIN", "FRONTEND_URL"]) ??
      defaultFrontendOrigin,
  );

export const getFrontendApiBase = (origin: string): string =>
  `${normalizeOrigin(origin)}/api`;
