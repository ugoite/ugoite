...
describe("createEntryStore", () => {
  ...
  it("sets error on load failure", async () => {
    server.use(
      http.get(
        testApiUrl("/spaces/es-ws/entries"),
  ...
  it("rolls back on update failure", async () => {
    await createRoot(async (dispose) => {
      ...
      server.use(
        http.put(
          testApiUrl(`/spaces/es-ws/entries/${entry.id}`),
          () => HttpResponse.json({ detail: "Error" }, { status: 500 }),
        ),
      );
      await expect(
  ...
  it("throws on search failure and sets error", async () => {
    server.use(
      http.get(
        testApiUrl("/spaces/es-ws/search"),
        () => HttpResponse.json({ detail: "Error" }, { status: 500 }),
      ),
    );
    await createRoot(async (dispose) => {
  ...
  it("handles RevisionConflictError when a different entry is selected", async () => {
    await createRoot(async (dispose) => {
      ...
      server.use(
        http.put(
          testApiUrl(`/spaces/es-ws/entries/${r1.id}`),
          () =>
            HttpResponse.json(
              { detail: "Conflict", revision_id: "server-rev" },
              { status: 409 },
            ),
        ),
      );
