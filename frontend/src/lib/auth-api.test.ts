...
describe("authApi", () => {
  ...
  it("REQ-OPS-015: surfaces local auth config and login response errors clearly", async () => {
    ...
    await expect(authApi.loginWithPasskeyTotp("dev-alice", "123456")).resolves
      .toEqual({
        userId: "dev-alice",
    ...
    await expect(authApi.loginWithPasskeyTotp("dev-alice", "123456")).rejects
      .toThrow(
  ...
  it("REQ-FE-066: loads and clears browser auth session state through the frontend session route", async () => {
    ...
    await expect(authApi.clearSession()).rejects.toThrow(
      "Failed to clear browser auth session.",
    );
  });
});
