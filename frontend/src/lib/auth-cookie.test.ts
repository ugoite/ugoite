...
import {
  ...
  buildServerAuthCookie,
  readAuthCookie,
} from "./auth-cookie";

describe("authCookie", () => {
  it("REQ-OPS-015: reads auth cookies from browser request headers", () => {
    expect(readAuthCookie("theme=dark; ugoite_auth_bearer_token=token%20value"))
      .toBe(
        "token value",
      );
  });
  ...
  it("REQ-OPS-015: treats missing, blank, and undecodable auth cookies safely", () => {
    ...
    expect(readAuthCookie("ugoite_auth_bearer_token=%E0%A4%A")).toBe(
      "%E0%A4%A",
    );
  ...
  it("REQ-OPS-015: omits auth cookie expiry when the browser session should stay transient", () => {
    const header = buildServerAuthCookie("token-value", undefined, {
      secure: false,
    });
  ...
  it("REQ-OPS-015: falls back to Date.now when the browser auth cookie expiry does not provide nowMs", () => {
    ...
    try {
      const header = buildServerAuthCookie("token-value", 1_900_000_000, {
        secure: false,
      });

      expect(header).toContain("Max-Age=60");
      expect(header).toContain(
        `Expires=${new Date(1_900_000_000 * 1000).toUTCString()}`,
      );
    } finally {
