...
const SQL_ERROR_PREFIX: &str = "UGOITE_SQL_ERROR";
...
pub fn parse_sql(query: &str) -> Result<SqlQuery> {
    ...
    {
        return Err(sql_error("Only SELECT * is supported in Ugoite SQL"));
    }
...
fn parse_limit_clause(limit_clause: &LimitClause) -> Result<Option<usize>> {
    match limit_clause {
        LimitClause::LimitOffset {
            ...
            if offset.is_some() {
                return Err(sql_error("OFFSET is not supported in Ugoite SQL"));
            }
            if !limit_by.is_empty() {
                return Err(sql_error("LIMIT BY is not supported in Ugoite SQL"));
            }
            match limit {
        ...
        LimitClause::OffsetCommaLimit { .. } => Err(sql_error(
            "LIMIT with comma offset is not supported in Ugoite SQL",
        )),
    }
}
...
fn parse_order_by(order_by: Option<&OrderBy>) -> Result<Vec<OrderByExpr>> {
    match order_by {
        ...
        Some(order_by) => match &order_by.kind {
            ...
            OrderByKind::All(_) => Err(sql_error("ORDER BY ALL is not supported in Ugoite SQL")),
        },
    }
}
