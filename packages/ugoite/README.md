...
For an actually published package on GitHub Packages, prefer a pinned version:

```bash
npx --package @ugoite/ugoite@<version> ugoite-install
```

Configure the scoped registry before installation:

```bash
npm config set @ugoite:registry https://npm.pkg.github.com
```
