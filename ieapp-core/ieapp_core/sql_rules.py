"""Shared IEapp SQL linting and completion rules."""
...
DEFAULT_RULES_PATH = (
    Path(__file__).resolve().parents[2] / "shared" / "sql" / "ieapp-sql-rules.json"
)
...
def _resolve_rules_path(path: str | None = None) -> Path:
    env_path = os.getenv("IEAPP_SQL_RULES_PATH")
    if path:
        return Path(path)
    if env_path:
...
def lint_sql(
    ...
    """Return lint diagnostics for an IEapp SQL query."""
    rules = rules or load_sql_rules()
