...
#[tokio::test]
/// REQ-LNK-004
async fn test_entry_req_lnk_004_normalize_ieapp_link_uris() -> anyhow::Result<()> {
    let op = setup_operator()?;
    ...
    let content = "---\nform: Entry\n---\n# Title\n\n## Body\nSee [ref](ieapp://entries/entry-123), [file](ieapp://assets/asset-456), and [query](ieapp://entry?id=entry-789).";
    ...
    assert!(content_info.markdown.contains("ieapp://entry/entry-123"));
    assert!(content_info.markdown.contains("ieapp://asset/asset-456"));
    assert!(content_info.markdown.contains("ieapp://entry/entry-789"));
