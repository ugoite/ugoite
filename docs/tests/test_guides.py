...
REQUIRED_CLI_INSTALLER_ASSET_FRAGMENTS = {
    ...
    "ugoite-v0.0.1-beta.15-aarch64-unknown-linux-gnu.install.sh",
    "ugoite-v0.0.1-beta.15-x86_64-apple-darwin.install.sh",
    "ugoite-v0.0.1-beta.15-aarch64-apple-darwin.install.sh",
}
...
def test_docs_req_sec_001_source_compose_requires_unique_dev_auth_secrets() -> None:
    """REQ-SEC-001: source Compose must stay loopback-only with unique auth secrets."""
    compose_text = (REPO_ROOT / "docker-compose.yaml").read_text(encoding="utf-8")
    ...
    if missing_guide_fragments:
        raise AssertionError(
            "docs/guide/docker-compose.md must document "
            "local-only unique auth secrets: " + ", ".join(missing_guide_fragments),
        )
...
def test_docs_req_ops_034_local_e2e_runner_default_signing_secret_length() -> None:
    ...
    if match is None:
        message = "run-e2e.sh must define a default UGOITE_DEV_SIGNING_SECRET"
        raise AssertionError(message)
