...
- Owner-approved Space access recovery is supported separately from account
  ...
  target completes a five-minute WebAuthn flow; a fresh HumanAccount is created
  and only the target Space Principal binding is replaced. Principal ID,
  membership, role, ACL, resource ownership, audit identity, old account
  credentials/sessions, and unrelated Space bindings remain unchanged.
- Node administrator and Space owner are separate roles.
- The owner token is one-use, its encrypted response copy is limited to the
  bounded one-time response boundary, and a new force-reset request deliberately
  supersedes the previous approval. Completion uses a persisted binding revision
  and recovery fence so stale, expired, revoked, or concurrent requests fail
  closed. No recovery token, code, TOTP secret, or hash is logged.
...
- CLI and agents register P-256 public keys. Access tokens are opaque,
  ...
  RFC 9449 DPoP. CLI REST requests omit `resource` and use the issuer audience;
  MCP requests use exactly `{issuer}/mcp` for both. DPoP `htu` excludes query
