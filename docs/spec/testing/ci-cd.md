...
11. **Container quick start** must stay documented in `README.md`, `docs/guide/container-quickstart.md`, and `docker-compose.release.yaml` so users can download the shipped compose file, prepare the documented env file, and pull/run published images without rebuilding from source, and those docs must keep a dedicated `Environment Variables` section aligned with the shipped release-compose overrides.
