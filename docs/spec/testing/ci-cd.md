...
export. Repository Dockerfiles pin external base/tool images to exact version tags, and base-image references should carry digests when public registries expose them, so local and CI builds stay reproducible under Dependabot-managed Docker update PRs.
