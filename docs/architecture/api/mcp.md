...
history semantics, and a storage encoding change must not silently change the
meaning of an MCP v1 operation.
...
- Save validation failures are returned as semantic tool errors. For example,
  ...
  missing or invalid Form fields. These errors include safe validation detail
  without exposing storage layout.
- `ugoite.undo` reverses all changes made by the current Konase Work through the
  existing Run undo semantic operation. Its empty model-facing argument object
  is unchanged; the Host supplies the Work Run ID in `_meta["ugoite/runId"]` for
  both `ugoite.save` and `ugoite.undo`.
...
| Saved SQL                                               | Not supported        | Use the authenticated REST/CLI Saved SQL and stateless SQL query surfaces; no MCP SQL tool is advertised.                                                                 |
...
Tools are filtered before `tools/list`: read-only credentials see search, write
credentials additionally see save and undo, and delete is shown only to an
...
published at `/.well-known/oauth-protected-resource`; authorization-server
...
same resource. Existing DPoP credentials may use `Authorization: DPoP` plus one
RFC 9449 proof. Agent credentials cannot use the MCP delete tool.
