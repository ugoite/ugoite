...
def get_entry(
    ...
    *,
    ...
    config = storage_config_from_root(root_path, fs)
    try:
        return run_async(ieapp_core.get_entry, config, space_id, safe_entry_id)
    except RuntimeError as exc:
        msg = str(exc)
        if "not found" in msg:
            raise FileNotFoundError(msg) from exc
...
def list_entries(
    ...
    *,
    ...
    root_path, space_id = split_space_path(space_path)
    config = storage_config_from_root(root_path, fs)
    return run_async(ieapp_core.list_entries, config, space_id)
