...
import json
import fsspec
from ieapp.indexer import extract_properties, validate_properties, aggregate_stats, Indexer, query_index
...
def test_indexer_run_once():
    ...
    assert "validation_warnings" not in index_data["note1"] or not index_data["note1"]["validation_warnings"]
